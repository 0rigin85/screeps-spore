...
export class UpgradeRoomController extends Task {
  ...
  constructor(public controller: Ptr<StructureController>) {
    ...
    if (!controller.isShrouded) {
      let area = <LookAtResultWithPos[]>(
        controller.instance.room.lookForByRadiusAt(LOOK_STRUCTURES, controller.instance, 4, true)
      );
      for (let pos of area) {
  ...
  hasWork(): boolean {
    ...
    if (this.labor.types[this.idealCreepBody.name] != null) {
      ...
      if (
        this.scheduledWork >= this.labor.types[this.idealCreepBody.name].parts[WORK] ||
  ...
  schedule(object: RoomObject): number {
    ...
    if (creep.type == CREEP_TYPE.UPGRADER.name) {
      ...
      let claimReceipt = this.claimable.makeClaim(
        this,
        RESOURCE_ENERGY,
